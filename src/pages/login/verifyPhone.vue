...
import {getVerifyPhoneCode,getVerifyCodeCheck} from '../../service/getData'
...
export default {
    ...
    methods:{
        ...
        async getCode() {
            var result = await getVerifyPhoneCode(this.codeParams);
            console.log(result)
            if(result.code==444){
                Toast({
        ...
        async checkCode(){
            var result = await getVerifyCodeCheck(this.phoneParams);
            if(result.code==444){
                Indicator.close();
                Toast({
                    message:result.datas.error,
...
<template>
<div class='wrapper'>
    <headerBack headTitle='Verify Phone'></headerBack>
    <ul class="loginList">
        <li class="loginUser">
            <i></i>
    ...
    <div class="nextButton" @click="nextClick">Verify Phone</div>

    <p class="informP">Once you verify your phone number
 you will receive Cash Rewards.</p>
