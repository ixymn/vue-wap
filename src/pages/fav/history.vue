<template>
  <div class="fav-box">
    <HEADE headTitle="History View" @goBackEvent="goBack" :isDiyBack="1"/>
    <div class="fav-main">
            ...
            <div v-infinite-scroll="scrollMore" infinite-scroll-disabled="loadScroll" infinite-scroll-distance="10">
              <dl class="fav-item" v-for="item in favArr">
                  ...
                  <dd class="fav-msg">
                    ...
                    <p class="fav-price">{{currencyUnit}} {{item.goods_promotion_price.split(".")[0]}}</p>
                    <div><span @click.prevent="favToCart(item.goods_id)" class="fav-cart">ADD TO CART</span></div>
                  </dd>
                  </router-link>
              </dl>
            </div>
...
import {getHistory,addToCart} from '../../service/getData';
  ...
  methods:{
    ...
    scrollMore:async function(){
      ...
      if(this.hasMore){
        ...
        let res=await getHistory(params,this.curPage);
        if(res.code == "200"){
          if(res.datas.goodsbrowse_list.length){
            this.favArr=this.favArr.concat(res.datas.goodsbrowse_list); 
    ...
    refresh:async function(){
      ...
      let res=await getHistory(params,1);
      if(res.code == "200"){
        this.favArr=res.datas.goodsbrowse_list;
      }
    },
